...
function AdminDashboardInner() {
  ...
  const handleImport = useCallback(
    (file: File, marque: string, modele: string) => {
      ...
      <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-50 px-6 py-4 flex items-center justify-between shadow-lg print:hidden">
        <div className="flex items-center gap-4">
          <Link to="/" className="font-black text-2xl tracking-tighter text-white flex items-center gap-2">
            La Truffe{" "}
            <span className="text-[10px] font-black text-indigo-300 bg-indigo-500/20 border border-indigo-500/30 px-3 py-1 rounded-lg uppercase tracking-widest">
              Master Admin
            </span>
          </Link>
        </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 bg-white rounded-xl shadow-xl border-slate-100 p-2">
              ...
              <DropdownMenuItem
                ...
                className="gap-2 font-bold text-red-600 cursor-pointer rounded-lg hover:bg-red-50"
              >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        <div className="bg-white border-b border-slate-200 px-6 print:hidden shadow-sm z-40 relative overflow-x-auto">
          <TabsList className="h-16 bg-transparent gap-8 max-w-7xl mx-auto w-full justify-start min-w-max">
            <TabsTrigger
              value="moderation"
              className="rounded-none h-full px-0 font-bold text-base data-[state=active]:text-rose-600 data-[state=active]:border-b-4 data-[state=active]:border-rose-600 data-[state=inactive]:text-slate-500 relative"
            >
            ...
            <TabsTrigger
              value="scanner"
              className="rounded-none h-full px-0 font-bold text-base data-[state=active]:text-indigo-600 data-[state=active]:border-b-4 data-[state=active]:border-indigo-600 data-[state=inactive]:text-slate-500"
            >
        ...
        <TabsContent
          value="moderation"
          className="flex-1 m-0 p-6 md:p-8 max-w-7xl mx-auto w-full space-y-8 animate-in fade-in"
        >
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
            <div>
              ...
              <p className="text-slate-500 font-medium mt-2">
                Contrôle des annonces déposées avant leur publication publique.
              </p>
            </div>
          ...
          <Card className="overflow-hidden border-slate-100 shadow-2xl rounded-[2.5rem] bg-white">
            <Table>
              <TableHeader className="bg-slate-50 border-b border-slate-100">
                <TableRow>
                  <TableHead className="font-black text-slate-500 uppercase tracking-widest text-[10px] py-5 pl-8">
                    Véhicule
                  </TableHead>
                  <TableHead className="font-black text-slate-500 uppercase tracking-widest text-[10px] py-5">
                    Vendeur
                  </TableHead>
                  <TableHead className="font-black text-slate-500 uppercase tracking-widest text-[10px] py-5">
                    Détails
                  </TableHead>
                  <TableHead className="text-right font-black text-slate-500 uppercase tracking-widest text-[10px] py-5 pr-8">
                    Actions
                  </TableHead>
                </TableRow>
              </TableHeader>
                ...
                {pendingListings.map((listing) => {
                  ...
                  return (
                    ...
                    >
                      <TableCell className="pl-8 py-5">
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            ...
                            <Badge className="bg-rose-100 text-rose-700 border-0 mt-2 hover:bg-rose-100 text-[10px] uppercase tracking-widest">
                              En attente
                            </Badge>
                          </div>
                        </div>
                      ...
                      <TableCell>
                        <p className="font-[1000] text-2xl text-emerald-600 tracking-tighter">
                          {safeNum(listing.prix)} €
                        </p>
                        ...
                        <p className="text-xs text-slate-400 mt-2 font-medium line-clamp-1 italic max-w-xs">
                          "{listing.description}"
                        </p>
                      </TableCell>
                      <TableCell className="text-right pr-8">
                        <div className="flex flex-col items-end gap-2">
                          <Button
                            ...
                            className="w-32 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-500/20"
                          >
                          ...
                          <Button
                            ...
                            className="w-32 border-rose-200 text-rose-600 hover:bg-rose-50 font-bold rounded-xl"
                          >
          ...
          {vehicles.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <p className="text-slate-500 text-lg mb-8 max-w-md">
                Importez un dataset CSV pour lancer l'algorithme d'analyse et repérer les anomalies du marché.
              </p>
              <Button
                onClick={() => setIsImportModalOpen(true)}
                className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold h-14 px-8 rounded-2xl shadow-xl shadow-indigo-600/20 active:scale-95 transition-all"
              >
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <Button
                      ...
                      className={`font-bold rounded-xl border-slate-200 ${isFiltersOpen ? "bg-slate-100 text-slate-900" : "text-slate-600 hover:bg-slate-50"}`}
                    >
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <Button
                      ...
                      size="sm"
                      onClick={clearData}
                      className="text-slate-500 hover:text-rose-600 font-bold rounded-xl"
                    >
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => setIsImportModalOpen(true)}
                      className="font-bold rounded-xl border-slate-200 text-slate-700"
                    >
                  ...
                  <div className="w-full mt-6 pt-6 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-10 animate-in slide-in-from-top-4">
                    <div className="space-y-4">
                      <Label className="font-bold text-slate-900 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-indigo-500" /> Prix : {filters.minPrice}€ -{" "}
                        {filters.maxPrice}€
                      </Label>
                      <Slider
                        value={[filters.minPrice, filters.maxPrice]}
                        onValueChange={(val) => setFilters({ minPrice: val[0], maxPrice: val[1] })}
                        min={0}
                    ...
                    <div className="space-y-4">
                      <Label className="font-bold text-slate-900 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-emerald-500" /> Kilométrage : {filters.minKm}km -{" "}
                        {filters.maxKm}km
                      </Label>
                      <Slider
                        value={[filters.minKm, filters.maxKm]}
                        onValueChange={(val) => setFilters({ minKm: val[0], maxKm: val[1] })}
                        min={0}
                    ...
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src =
                          `https://source.unsplash.com/1600x900/?car,${vehicleInfo?.marque}`;
                      }}
                    />
                ...
                <div className="w-full lg:w-2/3 flex flex-col justify-between">
                  <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl h-full">
                    <div className="flex flex-col md:flex-row md:items-start justify-between mb-10 gap-6">
                      <div>
                        <h1 className="text-4xl md:text-5xl font-[1000] text-slate-900 tracking-tighter mb-4">
                          {vehicleInfo?.marque} {vehicleInfo?.modele}
                        </h1>
                      ...
                      <div className="text-left md:text-right p-6 bg-slate-900 rounded-[2rem] text-white shadow-lg">
                        ...
                        <div className="text-5xl font-[1000] tracking-tighter">
                      ...
                      {[
                        {
                          ...
                          bg: "bg-indigo-50",
                          label: "Kilométrage",
                          value: `${safeNum(bestDeal?.kilometrage)} km`,
                        },
                        {
                          icon: Fuel,
                          ...
                          bg: "bg-amber-50",
                          label: "Carburant",
                          value: bestDeal?.carburant || "Essence",
                        },
                        {
                          icon: Calendar,
                        ...
                        <div
                          key={i}
                          className="bg-slate-50 p-5 rounded-3xl border border-slate-100 flex flex-col gap-3"
                        >
                          <div
        ...
        >
            ...
            <Card className="rounded-2xl border-slate-100 shadow-lg bg-white">
              <CardContent className="p-6 text-center">
                ...
                <p className="text-4xl font-black text-indigo-600">
                  {
                    allUsers.filter(
                      (u) => u.credits !== 1 || new Date(u.created_at) > new Date(Date.now() - 7 * 86400000),
                    ).length
                  }
                </p>
              </CardContent>
                    ...
                    >
                      ...
                      <TableCell className="text-right pr-8">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => fetchUserHistory(u.user_id)}
          ...
          {isUserModalOpen && (
            ...
            >
              <div
                className="bg-white rounded-[2rem] shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-8"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-2xl font-black text-slate-900">Historique Client</h3>
                  <Button variant="ghost" size="sm" onClick={() => setIsUserModalOpen(false)}>
                    <X className="w-5 h-5" />
                  </Button>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    {userReports.length === 0 ? (
                      <p className="text-slate-400 text-sm">Aucun rapport</p>
                    ) : (
